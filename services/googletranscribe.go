...
func (GoogleLLMNoteProcessor) ProcessAvatarTask(personAvatarPath string, modelName LLMModelName) (*LLMResponse, error) {
	...
	parts = append(parts, &genai.Part{
		...
		Text: "enerate a fashion-style full-body commercial head to toe photographer edited portrait of the  person from first image by keeping his identity, personality, facial identity(100% same) and use solid, flat, unlit, white second image as a new background for person image which will be chromakey. keep user facial identity exactly same, unchanged. By keeping same personality, identity and exact same body/hand/head/leg sizes - generate the straight facing the camera and relaxed, coolest, confident pose with neutral white shirt, white trousers and white neutral shoes. The lighting on user should be natural, soft and professional, high-resolution and opening the color of person. Remove items from hands, position neutrally with slight smile. Clean all background elements, watermarks, other people/objects. If no person detected: return \"NO_PERSON\", otherwise output only full-body person, with on flat, consistent, all white second image background. Do not apply slight grayish gradients, keep all edges white. Aspect ratio 9:16 portrait size",
	})
...
func (GoogleLLMNoteProcessor) GenerateTryOn(personAvatarPath string, filePaths []string, modelName LLMModelName) (*LLMResponse, error) {
	...
	result, err := client.Models.GenerateContent(ctx, modelName.String(), []*genai.Content{{Parts: parts}}, &genai.GenerateContentConfig{
		...
		SystemInstruction: &genai.Content{
			Parts: []*genai.Part{
				{Text: `Take the pants on the second image, shirt on third image, and shoes on fourth image and let the same exact woman from the first image wear it. generate image in e-commerce fashion style with white background`},
			},
		},
	})
	// client.Models.Cou
