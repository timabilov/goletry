...
func (GoogleLLMNoteProcessor) ProcessAvatarTask(personAvatarPath string, modelName LLMModelName) (*LLMResponse, error) {
	...
	parts = append(parts, &genai.Part{
		Text: "Generate a hyper-realistic fashion-style full-body head to toe e-commerce pose portrait of the  person from first image by keeping his identity, personality, facial identity(100% same) and use solid, flat, unlit, white second image as a new background for person image which will be chromakey. keep user facial identity exactly same, unchanged. User should stand on image friendly e-commerce pose. The lighting on user should be natural and professional, high-resolution. Remove items from hands, position neutrally with slight smile. Clean all background elements, watermarks, other people/objects. If no person detected: return \"NO_PERSON\", otherwise output only full-body person with white neutral shoes and slightly fashion pose, with on flat, consistent, all white second image background. Do not apply slight grayish gradients, keep all edges white. Aspect ratio 9:16 portrait size",
	})
